...
public class NoteWidget_4X4 extends AppWidgetProvider {
	private static final String TAG = "NoteWidget_4X4";
	...
	public void onUpdate(Context context, AppWidgetManager appWidgetManager,
		...
		if (c > 0) {
			for (int i = 0; i < c; i++) {
				...
				NoteWidget_4X4.updateAppwidget(context, appWidgetManager, appWidgetIds[i]);
			}
		}
	...
	public static void updateAppwidget(Context context, AppWidgetManager appWidgetManager, int appWidgetId) {
		...
		if (id != -1) {
			...
			views = new RemoteViews(context.getPackageName(), R.layout.widget_4x4_layout);
			Logs.d(TAG, "== > context.getPackageName():" + context.getPackageName());
			views.setTextViewText(R.id.widget_4x4_date_time, update_date + "\t" + update_time.substring(0, 5));
			views.setTextViewText(R.id.widget_4x4_textView, mContent);
			views.setImageViewResource(R.id.widget_4x4_imageView, mBackground);
			views.setImageViewResource(R.id.widget_4x4_header, new ChooseColor().headerBackground(mBackground));
			Logs.d(TAG, "== > 要显示的内容：" + mContent);
			views.setOnClickPendingIntent(R.id.widget_4x4_imageView, pendingIntent);
			if (views != null) {
